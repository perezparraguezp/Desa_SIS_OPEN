...
// Set some content to print
    ...
    for ($i = 1; $i <= $diasMes[$mes - 1]; $i++) {
        ...
        if ($semana != 0 && $semana != 6) {
            ...
            if (!$row2) {
                $sql3 = "select * from ausencia where dia='$diaGestion' and id_empleado='$id_empleado' limit 1";
    ...
    for ($i = 1; $i <= $diasMes[$mes - 1]; $i++) {
        ...
        if ($row2) {
            ...
            $total_marcas = 0; //No hay  marcas ese dia
        }
        if ($total_marcas >= 2) {
            ...
            if ($total_marcas == 1) {
                ...
                if ($total_marcas == 0) {
                    if ($semana != 0 && $semana != 6) {
                        $sql4 = "select * from ausencia inner join justificaciones using(id_justificacion)
                                        where dia='$diaGestion' and id_empleado='$id_empleado' limit 1";
                        $row4 = mysql_fetch_array(mysql_query($sql4));
                        if ($row4) {
                            $justificacion = $row4['codigo_justificacion'];
                            $codigoJustificaciones[$fila] = $row4['codigo_justificacion'] . ":" . $row4['nombre_justificacion'];
                            $fila++;
                        } else {
                            $entrada = "N/M";
                            $salida = "N/M";
        ...
        if ($semana != 0 && $semana != 6) {//Sabado y Domingo
            $html.='
                    <tr>
            ...
            $html.='
                    <tr style="background-color: #EEFCBB;">
                        <td style="width:30px;">' . $i . '</td>
                        <td style="width:60px;">' . $entrada . '</td>
                        <td style="width:60px;">' . $salida . '</td>
                        <td style="width:80px;">' . $justificacion . '</td>
            ...
            <td>
                <table border="1px">
                            <tr>
                               ...
                               <td style="width:150px;">' . $diasFaltantes . '</td>
                            </tr>
                                   ';

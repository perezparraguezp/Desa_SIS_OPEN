...
if($comunal==true){
    ...
    $sql_0 = "select *  from persona 
                                    ...
                                    where m_adulto_mayor='SI'
                                      AND minimental='$atributo' and persona.edad_total_dias>=65*12
    ...
    while ($row_0 = mysql_fetch_array($res_0)){
        ...
        if($row_1){
            $sql_2 = "select * from historial_parametros_am 
                                          where rut='$persona->rut' and indicador='minimental'
    ...
    if($establecimientos==true){
        ...
        while($row1 = mysql_fetch_array($res1)){
            ...
            while ($row_0 = mysql_fetch_array($res_0)){
                ...
                $json .= '{"IR":"'.$persona->rut.'","RUT":"'.$persona->rut.'","MINIMENTAL":"'.$persona->getParametro_AM('chile_cuida').'","NOMBRE":"'.$persona->nombre.'","EDAD":"'.$persona->edad.'","COMUNAL":"'.$persona->nombre_sector_comunal.'","ESTABLECIMIENTO":"'.$persona->nombre_centro_medico.'","SECTOR_INTERNO":"'.$persona->nombre_sector_interno.'","INDICADOR":"'.$indicador_json.'","anios":"'.$persona->edad_anios.'","meses":"'.$persona->edad_meses.'","dias":"'.$persona->edad_dias.'"}';
                $total_pacientes++;
                $json_coma++;
            }
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var setting = {
            title: 'MINIMENTAL - VALOR: <?php echo $atributo==''?'NO REGISTRA':$atributo; ?>',
        ...
        $("#table_grid").jqxGrid(
            {
                ...
                columns: [
                    ...
                    { text: 'MINIMENTAL', dataField: 'MINIMENTAL', cellsalign: 'center', width: 150,filtertype: 'checkedlist'},
                    { text: 'AÑOS', dataField: 'anios', cellsalign: 'center', width: 100},
        ...
        $("#excelExport").click(function () {
            $("#table_grid").jqxGrid('exportdata', 'xls', '<?php echo $TITULO_GRAFICO; ?>', true,null,true, 'https://www.sis-open.com/exportar/save-file.php');
        });

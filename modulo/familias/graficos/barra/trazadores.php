...
$sql = "select * from trazadores_familia where id_trazador='$atributo' limit 1";
$row = mysql_fetch_array(mysql_query($sql));
$nombre_atributo = $row['nombre_trazador'];
$indicador = 'FAMILIAS CON TRAZADOR - '.strtoupper($row['nombre_trazador']);

if($atributo=='VIGENTES'){
    $filtro = "and familia.fecha_vdi!='' and TIMESTAMPDIFF(DAY, fecha_vdi, current_date)<720 ";
}else{
    if($atributo=='NO VIGENTES'){
        $filtro = "and familia.fecha_vdi!='' and TIMESTAMPDIFF(DAY, fecha_vdi, current_date)>=720 ";
    }else{
        $filtro = "and familia.fecha_vdi=''";
    }
}
...
if($comunal==true){
    ...
    $sql_0 = "select *  from familia
                        inner join registro_trazadores_familia on familia.id_familia=registro_trazadores_familia.id_familia 
                        ...
                        and trazadores like '%;$atributo;%' group by familia.id_familia ";
    ...
    while ($row_0 = mysql_fetch_array($res_0)){
        ...
        $json .= '{"IR":"'.$familia->id_familia.'","ESTADO":" '.$nombre_atributo.'","CODIGO":"'.$familia->codigo_familia.'","NOMBRE":"'.$familia->nombre.'","COMUNAL":"'.$familia->nombre_sector_comunal.'","ESTABLECIMIENTO":"'.$familia->nombre_centro.'","SECTOR_INTERNO":"'.$familia->nombre_establecimiento.'"}';
    ...
    if($establecimientos==true){
        ...
        while($row1 = mysql_fetch_array($res1)){
            ...
            while ($row_0 = mysql_fetch_array($res_0)){
                ...
                $json .= '{"IR":"'.$familia->id_familia.'","ESTADO":"RIESGO '.$nombre_atributo.'","CODIGO":"'.$familia->codigo_familia.'","NOMBRE":"'.$familia->nombre.'","COMUNAL":"'.$familia->nombre_sector_comunal.'","ESTABLECIMIENTO":"'.$familia->nombre_centro.'","SECTOR_INTERNO":"'.$familia->nombre_establecimiento.'"}';

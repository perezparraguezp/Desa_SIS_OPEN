...
if($comunal==true){
    ...
    while ($row_0 = mysql_fetch_array($res_0)){
        ...
        $sql_1 = "select * from paciente_antecedentes_sm where rut='$persona->rut' limit 1";

        $row_1 = mysql_fetch_array(mysql_query($sql_1));
        if($row_1){
            $fecha = fechaNormal($row_1['fecha_ingreso']);
            $indicador_json = $row_1['nombre_antecedente'];
    ...
    if($establecimientos==true){
        ...
        while($row1 = mysql_fetch_array($res1)){
            ...
            while ($row_0 = mysql_fetch_array($res_0)){
                ...
                $row_1 = mysql_fetch_array(mysql_query($sql_1));
                if($row_1){
                    $sql_2 = "select * from historial_antecedentes_sm 
                                          where rut='$persona->rut' and indicador='$indicador'
                                          and TIMESTAMPDIFF(DAY,historial_antecedentes_sm.fecha_registro,CURRENT_DATE)<365

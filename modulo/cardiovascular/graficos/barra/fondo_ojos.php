...
if($atributo=='TODO'){
    $filtro = " and (valor='CON RETINOPATIA' OR valor='SIN RETINOPATIA' )";
}else{
    $filtro = " and valor='$atributo' ";
}

$sql_column = " sum(indicador='fondo_ojo' $filtro) as total_indicador,
                sum(indicador='fondo_ojo' and sexo='M' $filtro) as hombres,
                sum(indicador='fondo_ojo' and sexo='F' $filtro) as mujeres,
                sum(indicador='fondo_ojo' and (valor='CON RETINOPATIA' OR valor='SIN RETINOPATIA')) as total_sincon ";
...
$estado = 'EXAMEN AL DIA';
if($comunal==true){
    //total pacientes
    $sql_0 = "select *  from paciente_pscv 
                                    ...
                                    where m_cardiovascular='SI' and paciente_pscv.rut!='' 
                                    and patologia_dm='SI' ";
    ...
    while ($row_0 = mysql_fetch_array($res_0)){
        ...
        $sql_1 = "select * from pscv_diabetes_mellitus where rut='$persona->rut' limit 1";
        $row_1 = mysql_fetch_array(mysql_query($sql_1));
        if($row_1){
            $sql_2 = "select * from historial_diabetes_mellitus 
                                          where rut='$persona->rut' and indicador='fondo_ojo' 
            ...
            if($row_2){
                ...
                $indicador_json = 'FONDO OJOS ['.$row_1['fondo_ojo'].' - '.$fecha.']';
            }else{
                $fecha = '';
                $indicador_json = 'FONDO OJOS [PENDIENTE]';
            }
    ...
    $sql1 = "select 'GENERAL' as nombre_base,
                      ...
                      inner join persona on persona.rut=historial_diabetes_mellitus.rut
                      inner join paciente_establecimiento on persona.rut=paciente_establecimiento.rut
                      where TIMESTAMPDIFF(DAY,historial_diabetes_mellitus.fecha_registro,CURRENT_DATE)<365
    ...
    $hombres = $row1['hombres']!='' ?$row1['hombres']:0;
    $mujeres = $row1['mujeres']!='' ?$row1['mujeres']:0;

    if($atributo=='TODO'){
        $porcentaje_indicador = number_format(($total_indicador*100/$total_pacientes),0,'.','');
    }else{
        $total_pacientes = $row1['total_sincon'];
        $porcentaje_indicador = number_format(($total_indicador*100/$total_pacientes),0,'.','');
    }
    ...
    if($establecimientos==true){
        ...
        $sql1 = "select sector_comunal.nombre_sector_comunal as nombre_base,
                        sector_comunal.id_sector_comunal as id,
                                   $sql_column
                                    ...
                                    inner join centros_internos on sectores_centros_internos.id_centro_interno=centros_internos.id_centro_interno
                                    inner join sector_comunal on centros_internos.id_sector_comunal=sector_comunal.id_sector_comunal
                                    inner join historial_diabetes_mellitus  on historial_diabetes_mellitus.rut=persona.rut  
                                    where TIMESTAMPDIFF(DAY,historial_diabetes_mellitus.fecha_registro,CURRENT_DATE)<365
                                    and m_cardiovascular='SI' 
                                    AND (";
        ...
        while($row1 = mysql_fetch_array($res1)){
            ...
            $sql_0 = "select *  from paciente_pscv 
                                    ...
                                    inner join sector_comunal on centros_internos.id_sector_comunal=sector_comunal.id_sector_comunal
                                    where m_cardiovascular='SI' and paciente_pscv.rut!='' 
                                    and patologia_dm='SI' and sector_comunal.id_sector_comunal='$id '";
...
//            echo $sql1;
            ...
            while($row1 = mysql_fetch_array($res1)){
                ...
                $sql_0 = "select *  from paciente_pscv 
                                    ...
                                    inner join sector_comunal on centros_internos.id_sector_comunal=sector_comunal.id_sector_comunal
                                    where m_cardiovascular='SI' and paciente_pscv.rut!='' 
                                    and patologia_dm='SI' and centros_internos.id_centro_interno='$id '";
        ...
        group by sectores_centros_internos.id_sector_centro_interno';
            ...
            while($row1 = mysql_fetch_array($res1)){
                ...
                $sql_0 = "select *  from paciente_pscv 
                                    ...
                                    inner join sector_comunal on centros_internos.id_sector_comunal=sector_comunal.id_sector_comunal
                                    where m_cardiovascular='SI' and paciente_pscv.rut!='' 
                                    and patologia_dm='SI' and sectores_centros_internos.id_sector_centro_interno='$id '";
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var setting = {
            title: '<?php echo $TITULO_GRAFICO.' - '.$estado; ?>',

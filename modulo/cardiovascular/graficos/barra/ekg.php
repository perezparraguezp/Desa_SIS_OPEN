...
$sql_column = " sum(indicador='ekg' and  TIMESTAMPDIFF(DAY,historial_parametros_pscv.fecha_registro,CURRENT_DATE)<365) as total_vigente,
                sum(indicador='ekg' and  TIMESTAMPDIFF(DAY,historial_parametros_pscv.fecha_registro,CURRENT_DATE)>=365) as total_no_vigente,
                sum(indicador='ekg' and sexo='M' and TIMESTAMPDIFF(DAY,historial_parametros_pscv.fecha_registro,CURRENT_DATE)<365) as hombres_vigentes,
                sum(indicador='ekg' and sexo='F' and TIMESTAMPDIFF(DAY,historial_parametros_pscv.fecha_registro,CURRENT_DATE)<365) as mujeres_vigentes,
                sum(indicador='ekg' and sexo='M' and TIMESTAMPDIFF(DAY,historial_parametros_pscv.fecha_registro,CURRENT_DATE)>365) as hombres_no_vigentes,
                sum(indicador='ekg' and sexo='F' and TIMESTAMPDIFF(DAY,historial_parametros_pscv.fecha_registro,CURRENT_DATE)>365) as mujeres_no_vigentes ";
...
if($comunal==true){
    ...
    while ($row_0 = mysql_fetch_array($res_0)){
        ...
        $sql_1 = "select * from parametros_pscv where rut='$persona->rut' limit 1";
        $row_1 = mysql_fetch_array(mysql_query($sql_1));
        if($row_1){
            $sql_2 = "select * from historial_parametros_pscv 
                                          where rut='$persona->rut' and indicador='ekg'
    ...
    $sql1 = "select 'GENERAL' as nombre_base,
                      ...
                      inner join persona on persona.rut=historial_parametros_pscv.rut
    ...
    if($atributo=='VIGENTE'){
        ...
        $total_indicador = $total_pendiente;
        $hombres = $hombres_pendientes;
        $mujeres = $mujeres_pendientes;
    }
        ...
        $sql1 = "select sector_comunal.nombre_sector_comunal as nombre_base,
                        sector_comunal.id_sector_comunal as id,
                                   $sql_column
                                    ...
                                    inner join centros_internos on sectores_centros_internos.id_centro_interno=centros_internos.id_centro_interno
                                    inner join sector_comunal on centros_internos.id_sector_comunal=sector_comunal.id_sector_comunal
                                    inner join historial_parametros_pscv  on historial_parametros_pscv.rut=persona.rut  
                                    where TIMESTAMPDIFF(DAY,historial_parametros_pscv.fecha_registro,CURRENT_DATE)<365
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#excelExport").click(function () {
            $("#table_grid").jqxGrid('exportdata', 'xls', 'PSCV EKG', true,null,true, 'https://carahue.eh-open.com/exportar/save-file.php');
        });
